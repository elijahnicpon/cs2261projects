...
void updateAndDrawHazards() {
    for (int i = 0; i < NUM_HAZARDS; i++) {
        if (hazards[i].active) {
            shadowOAM[hazards[i].oam_entry].attr0 = (hazards[i].y & (0xFF)) | (0 << 13) | (hazards[i].isTall) ? (2 << 14) : 0 | (hazards[i].isWide) ? (1 << 14) : 0;
            shadowOAM[hazards[i].oam_entry].attr1 = (hazards[i].x & (0x1FF)) | (hazards[i].size == 1) ? (1 << 14) : 0 | (hazards[i].size == 2) ? (2 << 14) : 0 | (hazards[i].size == 3) ? (3 << 14) : 0;

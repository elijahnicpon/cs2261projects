...
void goGame(int seed) {
    state = GAME;
    DMANow(3, pikachuPal, ((unsigned short *)0x5000000), 512);
    fillScreen4(1);
    time = 0;
    srand(seed);
    initGame();
...
void drawBalls() {
    for (int i = 0; i < BALLCOUNT; i++) {
        ...
        if (collision(balls[i].x, balls[i].y, balls[i].size, balls[i].size, player.x+1, player.y+2, player.width-2, player.height-4)) {
            gameover = 1;
        }
    }
}
...
void checkButtons() {
    if (player.dy == 1 && (!(~(oldButtons) & ((1<<6))) && (~buttons & ((1<<6))))) {
        player.dy = -1;

        *(volatile u16*)0x04000068 = (((3) & 15) << 12) | (0 << 11) | (((0) & 7) << 8) | (2 << 6);
        *(volatile u16*)0x0400006C = NOTE_D4 | (1<<15) | (1<<14);
    }
    if (player.dy == -1 && (!(~(oldButtons) & ((1<<7))) && (~buttons & ((1<<7))))) {
        player.dy = 1;

        *(volatile u16*)0x04000068 = (((3) & 15) << 12) | (0 << 11) | (((0) & 7) << 8) | (2 << 6);
        *(volatile u16*)0x0400006C = NOTE_D3 | (1<<15) | (1<<14);
    }
